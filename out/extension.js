...
class FilterFileSystem {
    ...
    async writeFile(uri, bytes, options) {
        const key = uri.toString();
...
function activate(context) {
    ...
    const sourceAt = (session, projectedLine, character) => {
    ...
    const projectedAt = (session, source) => {
        const candidates = session.sourceRows.flatMap((row, line) => row?.uri === source.uri ? [{ row, line }] : []);
        if (candidates.length === 0) {
            return undefined;
        }
        const candidate = candidates.find(({ row }) => row.line >= source.line) ??
            candidates[candidates.length - 1];
        return { line: candidate.line, character: source.character };
    };
    for (const stored of context.workspaceState.get(storedSessionsKey, [])) {
        ...
        const virtualUri = makeVirtualUri(stored.id, stored.sourceUri ?? stored.rootUri ?? 'search');
        const session = {
            ...stored,
            kind,
            virtualUri,
    ...
    const persist = async () => {
        const stored = [...sessions.values()].map(({ id, kind, sourceUri, rootUri, query, languageId, matchCase, wholeWord, useRegex, }) => ({
            id,
            kind,
            sourceUri,
            rootUri,
    ...
    const logState = (reason) => {
        output.appendLine(`\n=== ${reason} ===`);
        output.appendLine(`Editor inset API: ${typeof vscode.window.createWebviewTextEditorInset === 'function' ? 'available' : 'unavailable (using Quick Input fallback)'}`);
        output.appendLine(JSON.stringify([...sessions.values()].map((session) => ({
            id: session.id,
            ...
            sourceLocations: session.sourceRows.flatMap((row) => row ? [`${row.uri}:${row.line + 1}`] : []),
            message: session.message,
        })), null, 2));
    };
    const decorateEditor = (editor) => {
        ...
        editor.setDecorations(lineNumberDecoration, session.sourceRows.flatMap((row, projectedLine) => row
            ? [
                {
                    range: new vscode.Range(projectedLine, 0, projectedLine, 0),
                    renderOptions: {
                        before: {
                            contentText: (0, projection_1.formatSourceLineNumber)(row.line, session.sourceLineCount),
                        },
                    },
            ...
            : []));
        editor.setDecorations(fileHeaderDecoration, session.headers.map((header) => ({
    ...
    const updateStatus = () => {
        ...
        const source = session &&
            sourceAt(session, editor.selection.active.line, editor.selection.active.character);
        status.text = source
    ...
    const captureAnchors = (session) => {
        return vscode.window.visibleTextEditors
            .filter((editor) => editor.document.uri.toString() === session.virtualUri.toString())
            .map((editor) => ({
            editor,
            selections: editor.selections.flatMap((selection) => {
                const anchor = sourceAt(session, selection.anchor.line, selection.anchor.character);
                const active = sourceAt(session, selection.active.line, selection.active.character);
                return anchor && active ? [{ anchor, active }] : [];
            }),
            visibleStart: editor.visibleRanges[0]
                ? sourceAt(session, editor.visibleRanges[0].start.line, 0)
                : undefined,
        }));
    };
    const restoreAnchors = (session) => {
        const anchors = pendingAnchors.get(session.id) ?? [];
        if (anchors.some((anchor) => anchor.editor.document.getText() !== session.content)) {
        ...
        for (const anchor of anchors) {
            const selections = anchor.selections.flatMap((selection) => {
                ...
                const startLine = anchor.editor.document.lineAt(Math.min(start.line, anchor.editor.document.lineCount - 1));
                const endLine = anchor.editor.document.lineAt(Math.min(end.line, anchor.editor.document.lineCount - 1));
                return [
                    new vscode.Selection(start.line, Math.min(start.character, startLine.text.length), end.line, Math.min(end.character, endLine.text.length)),
                ];
            });
            if (selections.length > 0) {
                anchor.editor.selections = selections;
            }
            const visible = anchor.visibleStart &&
                projectedAt(session, anchor.visibleStart);
            if (visible && anchor.editor.document.lineCount > 0) {
                anchor.editor.revealRange(new vscode.Range(visible.line, 0, visible.line, 0), vscode.TextEditorRevealType.AtTop);
            }
        }
    ...
    const renderProjectionWhenCurrent = (session, generation, attempt = 0) => {
        if (generation !== session.generation) {
            return;
        }
        const editors = vscode.window.visibleTextEditors.filter((editor) => editor.document.uri.toString() === session.virtualUri.toString());
        if (editors.length === 0) {
            return;
        }
        if (editors.some((editor) => editor.document.getText() !== session.content)) {
            if (attempt < 50) {
                session.renderTimer = setTimeout(() => renderProjectionWhenCurrent(session, generation, attempt + 1), 10);
            }
            return;
        }
        restoreAnchors(session);
    };
    const scheduleProjectionRender = (session, generation) => {
        if (session.renderTimer) {
            clearTimeout(session.renderTimer);
        }
        session.renderTimer = setTimeout(() => renderProjectionWhenCurrent(session, generation), 0);
    };
    const getFinder = async (rootUri) => {
    ...
    const refresh = async (session) => {
        const openProjection = vscode.workspace.textDocuments.find((document) => document.uri.toString() === session.virtualUri.toString());
        if (openProjection?.isDirty) {
            if (!session.refreshAfterSave) {
                void vscode.window.showInformationMessage('Save the edited filtered document before changing its filter.');
            }
        ...
        try {
            if (session.kind === 'file') {
                ...
                const source = await vscode.workspace.openTextDocument(vscode.Uri.parse(session.sourceUri));
                if (generation !== session.generation) {
                    return;
                }
                const projection = (0, projection_1.projectLines)(source.getText(), session.query, {
                ...
                if (!session.query) {
                    ...
                    const projection = (0, projection_1.projectSearchMatches)(result.value.items.map((match) => ({
                    ...
                    session.sourceLineCount = Math.max(1, ...projection.rows.flatMap((row) => (row ? [row.line + 1] : [])));
    ...
    const scheduleRefresh = (session, force = false) => {
        if (!force &&
    ...
    const revealSourceLineAtTop = (editor, session, sourceLine) => {
        const projected = (0, projection_1.toProjectedPosition)({ line: sourceLine, character: 0 }, session.sourceLines);
        if (projected) {
            editor.revealRange(new vscode.Range(projected.line, 0, projected.line, 0), vscode.TextEditorRevealType.AtTop);
        }
    };
    const showFilterInput = (session, restoreSourceLine) => {
        ...
        input.onDidHide(() => {
            ...
            if (restoreSourceLine !== undefined) {
                setTimeout(() => {
                    const editor = vscode.window.visibleTextEditors.find((candidate) => candidate.document.uri.toString() === session.virtualUri.toString());
                    if (editor) {
    ...
    const ensureFilterInset = (editor, session, focusInput) => {
        ...
        try {
            ...
            const inset = vscode.window.createWebviewTextEditorInset(editor, firstVisibleLine - 1, 2, { enableScripts: true });
            filterInsets.set(editor, { sessionId: session.id, inset });
            inset.webview.html = filterInsetHtml(inset.webview, session, focusInput);
            const messageSubscription = inset.webview.onDidReceiveMessage((message) => {
            ...
            context.subscriptions.push(inset, messageSubscription, disposeSubscription);
            return true;
        }
        catch (error) {
            output.appendLine(`Editor inset unavailable; falling back to Quick Input: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    };
    const showSession = async (session, sourceVisibleLine, focusFilterInput = false) => {
        const document = await vscode.workspace.openTextDocument(session.virtualUri);
        const languageDocument = document.languageId === session.languageId
            ? document
            : await vscode.languages.setTextDocumentLanguage(document, session.languageId);
    ...
    provider.setWriteHandler(async (uri, content) => {
        ...
        const plan = (0, projection_1.planProjectionSave)(session.content, content, session.sourceRows);
        if (!plan.ok) {
        ...
        for (const edit of plan.edits) {
            ...
            if (edit.line >= document.lineCount ||
        ...
        if (plan.edits.length > 0) {
            ...
            for (const edit of plan.edits) {
                ...
                workspaceEdit.replace(document.uri, document.lineAt(edit.line).range, edit.after);
            }
            if (!(await vscode.workspace.applyEdit(workspaceEdit))) {
                throw vscode.FileSystemError.Unavailable('VS Code could not apply the projected source edits.');
            }
            for (const document of sourceDocuments.values()) {
                if (!(await document.save())) {
                    throw vscode.FileSystemError.Unavailable(`Could not save ${document.uri.fsPath}`);
                }
            }
        }
        session.content = content;
        const savedLines = content.split(/\r?\n/);
        if (savedLines.length === session.sourceRows.length + 1 &&
            savedLines.at(-1) === '') {
        ...
        await persist();
        if (plan.edits.length > 0) {
            void vscode.window.setStatusBarMessage(`Filter Lines: saved ${plan.edits.length} projected line${plan.edits.length === 1 ? '' : 's'} to ${sourceDocuments.size} file${sourceDocuments.size === 1 ? '' : 's'}`, 3_000);
        }
        if (refreshAfterSave) {
            setTimeout(() => scheduleRefresh(session, true), 0);
        }
    });
    context.subscriptions.push(provider, output, status, lineNumberDecoration, fileHeaderDecoration, {
        dispose: () => {
        ...
        isReadonly: false,
    }), vscode.commands.registerCommand('editor-filter.filterThisFile', async () => {
        const sourceEditor = vscode.window.activeTextEditor;
        if (!sourceEditor || sourceEditor.document.uri.scheme === scheme) {
            void vscode.window.showWarningMessage('Open a source file before creating a filtered view.');
            return;
        }
        const id = (0, node_crypto_1.randomUUID)();
        const sourceUri = sourceEditor.document.uri.toString();
        const initialQuery = sourceEditor.document.getText(sourceEditor.selections[0]);
        const sourceVisibleLine = sourceEditor.visibleRanges[0]?.start.line ??
        ...
        const hasInset = await showSession(session, sourceVisibleLine, !initialQuery);
        if (!initialQuery && !hasInset) {
            showFilterInput(session, sourceVisibleLine);
        }
    }), vscode.commands.registerCommand('editor-filter.searchProject', async () => {
        const sourceEditor = vscode.window.activeTextEditor;
        const sourceDocument = sourceEditor?.document.uri.scheme === scheme
            ? undefined
        ...
        if (!workspaceFolder || workspaceFolder.uri.scheme !== 'file') {
            void vscode.window.showWarningMessage('Open a local workspace folder before searching the project.');
            return;
        }
        const id = (0, node_crypto_1.randomUUID)();
        ...
        const session = {
            ...
            wholeWord: false,
            useRegex: false,
            virtualUri: makeVirtualUri(id, vscode.Uri.joinPath(workspaceFolder.uri, 'Project Search').toString()),
            content: '',
            sourceLines: [],
            sourceRows: [],
            headers: [],
        ...
        const source = sourceAt(session, editor.selection.active.line, editor.selection.active.character);
        if (!source) {
            return;
        }
        ...
        const character = Math.min(source.character, document.lineAt(line).text.length);
        const selection = new vscode.Selection(line, character, line, character);
        const viewColumn = editor.viewColumn;
        if (vscode.window.activeTextEditor?.document.uri.toString() ===
        ...
        sourceEditor.revealRange(sourceEditor.selection, vscode.TextEditorRevealType.InCenterIfOutsideViewport);
...
function filterInsetHtml(webview, session, focusInput) {
    ...
    const placeholder = session.kind === 'project'
...
function sessionId(uri) {
    return new URLSearchParams(uri.query).get('session') ?? '';
}
function makeVirtualUri(id, sourceUri) {
